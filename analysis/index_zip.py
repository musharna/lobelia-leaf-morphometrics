...
scanned = 0
saved = 0
idx = open(index_out, "w", encoding="utf-8")
while True:
    f.seek(off)
    hdr = f.read(30)
    ...
    if wanted(name, usz):
        ...
        try:
            ...
            sys.stderr.write("DECOMP_FAIL %s %s\n" % (name, e))
            off = data_off + csz
            continue
    ...
    if scanned % 25000 == 0:
        sys.stderr.write("scanned=%d off=%d saved=%d\n" % (scanned, off, saved))
        idx.flush()

    off = data_off + csz

idx.close()
sys.stderr.write("DONE scanned=%d saved=%d final_off=%d\n" % (scanned, saved, off))

...
d = np.load("/home/mjarnold/.claude/jobs/52463446/tmp/outlines.npz", allow_pickle=True)
...
for ax, sp in zip(axes.ravel(), order):
    ...
    ax.text(0.97, 0.05, f"n={m.sum()}  ({len(set(v[m]))} specimens)", transform=ax.transAxes,
...
fig.supxlabel(f"PC1 — {var[0]*100:.0f}% of shape variance (leaf breadth relative to length)",
              fontsize=10.5, color=INK, y=0.045)
fig.supylabel(f"PC2 — {var[1]*100:.0f}%", fontsize=10.5, color=INK, x=0.012)
fig.suptitle("Leaf shape space, one panel per species (grey = all 486 leaves)",
             fontsize=12.5, color=INK, y=0.985)
fig.tight_layout(rect=[0.02, 0.05, 1, 0.96])
out = "/home/mjarnold/.claude/jobs/52463446/tmp/lobelia_shape_space.png"
fig.savefig(out, dpi=155, facecolor="white")
print("wrote", out, "| PC1", round(var[0]*100,1), "% PC2", round(var[1]*100,1), "% | n", len(y))
